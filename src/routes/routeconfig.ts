...
const namedLazy = <T extends Promise<Record<string,Component>>>(componentLoader: () => T, name: keyof Awaited<T>) => {
    return lazy(async () => {
        return {
...
export const router: RouteDefinition[] = [
    ...
    {
        path: "/button",
        component: namedLazy(() => import("../routes/ButtonClicker"), "ButtonClicker"),
    }
];
